  ...
  return (
    ...
    <nav>
      <ul className='flex font-semibold text-gray-600'>
        <li className="li1" >
          <Link to='/charts'>CryptoVola</Link>
        </li>
      ...
      <ul  className='flex items-center'>
        {formData.email === "" ?  <Link to={"/login"}><li className='mr-6 p-3 text-yellow-300 text-lg font-bold hover:text-yellow-200' onClick={handleLogin}>Log In</li></Link> : <p className="profile-name text-white font-semibold mr-6 p-3">{formData.firstName} {formData.lastName}</p> }
